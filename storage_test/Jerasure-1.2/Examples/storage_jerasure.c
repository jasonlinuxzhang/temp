...
char *get_data(long data_index)
{
    char *pdata = NULL;
    pdata = (char *)malloc(DATA_UNIT * sizeof(char));
    return pdata;
}
...
void * handle_encode(void *argv)
{
    ...
    for(data_index = get_index(); data_index < DATA_NUM; data_index = get_index())
    {
        for(i = 0; i < _D; i++)
        {
            databuf[_M] = get_data(data_index);
            for(j = 0; j < _M; j++)
                enc_matrix[(j + 1) * (_M + 1) - 1] = fullEncMatric[j][i];
        
            jerasure_matrix_encode(_M + 1, _M, 8, enc_matrix, databuf, codebuf, DATA_UNIT);
